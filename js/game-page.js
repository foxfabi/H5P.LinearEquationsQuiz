...
H5P.LinearEquationsQuiz.GamePage = (function ($, UI, EquationType, QuestionsGenerator) {
  ...
  function GamePage(equationType, useFractions, maxValue, maxQuestions, t, id) {
    ...
    self.timer = new H5P.LinearEquationsQuiz.TimerWidget(t);
    self.timer.appendTo(self.$gamepage);

    self.slider = UI.createSlider();

    self.countdownWidget = new H5P.LinearEquationsQuiz.CountdownWidget(4, t);
    ...
    // Add result page:
    self.resultPage = new H5P.LinearEquationsQuiz.ResultPage(numQuestions, t);
  ...
  GamePage.prototype.createSlide = function (question, i) {
    ...
    var readableArithmetic = question.expression;

    //var readableQuestion = self.translations.humanizedQuestion
    //    .replace(':arithmetic', question.variable);

    var questionId = 'linearequation-quiz-' + self.id + '-question-' + i;
    ...
    alternatives.forEach(function (alternative, index) {
      ...
      alternative.on('answered', function () {
        ...
        // FIXME: H5P.LinearEquationsQuiz.SoundEffects.play(alternative.correct ? 'positive-short' : 'negative-short', 300);
